...
public class Commands implements CommandExecutor {
	...
	private void sendDefaultHelp(Player p) {
		p.sendMessage("§8§l> §9§lAdvanceParticle §7plugin by: §c§lTixius24");
	...
	private void sendHelp(Player p) {
		p.sendMessage("§8=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=");
		p.sendMessage("§8> §7Plugin version: §9" + plugin.getDescription().getVersion() + " §7Developed by §cTixius24");
		...
		p.sendMessage("§8> ");
		p.sendMessage("§8> §7Plugin website: §chttps://www.spigotmc.org/resources/71929/");
		p.sendMessage("§8=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=");
	}
